...
export default function Profile() {
  ...
  formData.append("upload_preset", "dp3llhyf"); // Replace with your actual upload preset
try {
  // Send POST request to Cloudinary API
  const response = await fetch(
    `https://api.cloudinary.com/v1_1/dbgbh10mc/upload`,
    {
      method: "POST",
      body: formData,
    }
  ...
  const downloadURL = data.secure_url; // Get the image URL from Cloudinary

  // Update the formData state with the Cloudinary image URL
    ...
    <div className="p-3 max-w-lg mx-auto">
      ...
      {userListings.map((listing) => (
        <div key={listing._id} className="border rounded-lg p-3 flex justify-between items-center gap-4">
          ...
          <img src={listing.imageUrls[0]} alt='liating cover' className="h-16 w-16 object-contain "/>
          </Link>
